...
class _CameraHomeState extends State<CameraHome> with WidgetsBindingObserver, TickerProviderStateMixin {
  ...
  int _selectedTimerOption = 0;
  double opacityValue = 1.0;
  IconData flashIcon = Icons.flash_off;
  IconData timerIcon = Icons.timer_off;
  int currentTimerValue = 0; // Current value of the countdown ticker
  ...
  Widget createOverlay() {
    Animation<double> opacity = AlwaysStoppedAnimation(opacityValue);

    Image overlay = imageFile == null
        ? Image.network(
            'https://www.clipartmax.com/png/middle/300-3007517_naruto-chibi-by-abaoabao-on-deviantart-anime-naruto-chibi-png.png',
            opacity: opacity,
            fit: BoxFit.cover,
          )
        : Image.file(
        ...
        children: [
          ...
          IconButton(
            onPressed: () {
              setState(() {
                rowState = 0;
                _selectedTimerOption = 3;
                timerIcon = Icons.timer_3;
                buildRow();
              });
            },
            color: Colors.white,
          ...
          IconButton(
            onPressed: () {
              setState(() {
                rowState = 0;
                _selectedTimerOption = 10;
                timerIcon = Icons.timer_10;
                buildRow();
              });
            },
            color: Colors.white,
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        body: Column(
          children: <Widget>[
            Container(width: double.infinity, height: 100.0, color: Colors.black, child: buildRow()),
            Expanded(
              child: Container(
    ...
    return Listener(
      ...
      child: Stack(
        children: [
          ...
          if (isOverlayActive) createOverlay(),
        ],
      ),
    );
  }
            ...
            child: Row(
              ...
              children: [
                ...
                Container(
                  ...
                  height: 80,
                  child: IconButton(
                    icon: Image.asset('assets/ShutterIcon.png'),
                    color: Colors.blue,
                    onPressed: cameraController != null && cameraController.value.isInitialized ? startTimer : null,
                  ),
                ),
                IconButton(
